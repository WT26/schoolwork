...
string Yksikko::tulosta_kohdeyksikot_kohdassa(int kohta){
    return kohdeyksikot_[kohta].tulosta();
}
        ...
        while(indeksi != kaikki_yksikot.size()){
            for(unsigned int i{0};i != lapi_kaydyt.size();i++){
                for(unsigned int i2{0};i2 != loytyy_listasta.size();i2++){
                    if((!kaikki_yksikot[indeksi].vertaa_yksikon_nimea(lapi_kaydyt[i]) )
                            && (kaikki_yksikot[indeksi].vertaa_yksikon_nimea(loytyy_listasta[i2]))){
